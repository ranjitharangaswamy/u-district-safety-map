              ...
              <div className="h-96 bg-gradient-to-br from-green-100 to-blue-100 relative">
                ...
                {incidents.map((incident, index) => {
                  ...
                  return (
                    <div
                      key={incident.id}
                      className={`absolute w-8 h-8 rounded-full flex items-center justify-center cursor-pointer transform -translate-x-1/2 -translate-y-1/2 ${'bg-' + (
                        incident.severity === 'high' ? 'red' : 
                        incident.severity === 'medium' ? 'orange' : 'yellow'
                      ) + '-500'} text-white shadow-lg hover:scale-110 transition-transform`}
                      style={{
                        left: `${20 + index * 25}%`,
                        top: `${30 + index * 15}%`
                      }}
